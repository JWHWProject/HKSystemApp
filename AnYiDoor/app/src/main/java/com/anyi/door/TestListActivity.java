...
//                        NetLoadingDialog.getInstance().loading(TestListActivity.this, "提交中,请稍后");
    ...
    public void onEventMainThread(BaseResponse event) throws JSONException
    {
        if (event instanceof NoticeEvent)
        {
            ...
            if (NotiTag.TAG_CLOSE_WITH_UEL.equals(tag) && BaseApplication.currentActivity.equals(this.getClass().getName()))
            {
                if (null != mFinishTestResponse && GeneralUtils.isNotNullOrZeroLenght(mFinishTestResponse.getUrl()))
                {
                    Intent intentDoFile = new Intent(mContext, CommonWebViewActivity.class);
                    intentDoFile.putExtra(IntentCode.COMMON_WEB_VIEW_TITLE, "考核结果");
                    intentDoFile.putExtra(IntentCode.COMMON_WEB_VIEW_URL, mFinishTestResponse.getUrl());
